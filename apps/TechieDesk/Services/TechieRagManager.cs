...
public class TechieRagManager : ITechieRag, IDisposable
{
    ...
    private async Task<ITechieRag> CreateInstanceFromConfigAsync()
    {
        ...
        if (savedConfig != null)
        {
            ...
            // Wave 3 (REQ-RAG-014/REQ-RAG-025): enable the optional rerank stage when configured.
            if (savedConfig.Rerank is { Enabled: true } rerank &&
                rerank.Source is RerankSource.Cohere or RerankSource.Jina &&
        ...
        // (IConversationStore). The store lives in its OWN SQLite file, separate from both the
        // app DB (DbUp-owned) and the SqliteVec vector store, so the three never contend.
        var dataDirectory = Path.Combine(_environment.ContentRootPath, "data");
        Directory.CreateDirectory(dataDirectory);
        var persistenceDbPath = Path.Combine(dataDirectory, "techiedesk-rag-store.db");
    ...
    public async Task<WorkspaceManager?> GetWorkspaceManagerAsync()
    {
        var instance = await GetInstanceAsync();
    ...
    WorkspaceManager? ITechieRag.GetWorkspaceManager()
        => _currentInstance?.GetWorkspaceManager();

    /// <inheritdoc />
    IConversationStore? ITechieRag.GetConversationStore()
        => _currentInstance?.GetConversationStore();
