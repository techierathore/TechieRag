...
| REQ-FN-057 | F-PLATFORM: CI builds the probe for all four heads on every push and runs its button on an Android emulator where CI allows; an unbuildable head is reported, never skipped | PARTIAL | 75% | [REQ-FN-057] 2026-09-25 CI run 36148218069: Windows and Android built and both buttons passed; Mac Catalyst and iOS failed because macos-15 has no Xcode 26.5, which .NET for iOS 26.5 requires. Fixed: Apple jobs on macos-26 with `scripts/select-xcode.sh`, failed builds annotate the run page; test `AppleJobsSelectTheXcodeTheirSdkAsksFor`. Waiting on the next push's run page | [view](#d-req-fn-057) |
...
| REQ-FN-060 | F-LOCAL-LLM: tokens per second, time to first token and peak memory recorded per platform on named devices in the support matrix | PARTIAL | 75% | [REQ-FN-060] 2026-09-25 Windows, Mac Catalyst, Android (Galaxy S23) and the iOS simulator recorded. iPhone 17 Pro connected; its device build passes (`tests/.artifacts/probe/ios/build-device-arm64-nosign.log`) but deploying waits on the owner: Developer Mode is off on the phone and no Apple ID is signed in to Xcode, so no development certificate exists | [view](#d-req-fn-060) |
