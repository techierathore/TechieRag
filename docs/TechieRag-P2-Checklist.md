...
| REQ-RAG-044 | The shipped `IVectorStore` set is SQLite, pgvector and Qdrant; `PgVectorStore` shall be proven against a real PostgreSQL through `LivePgVectorStoreTes | Verified | 100% | 2026-09-25 verify: PASS — test REQ-RAG-044 DeletingADocumentRemovesItsVectorsFromSearch | [view](#d-req-rag-044) |
...
| REQ-FN-057 | F-PLATFORM: CI builds the probe for all four heads on every push and runs its button on an Android emulator where CI allows; an unbuildable head is reported, never skipped | PARTIAL | 75% | [REQ-FN-057] 2026-09-25 CI run 36134404881 failed Android, Mac Catalyst and iOS at restore: `-f` still restored every head, so Linux needed the iOS SDK and macOS the Android workload. Fixed: the probe takes `-p:ProbeHead=<framework>` and probe.yml passes it; test `ProbeHeadNarrowsRestoreToOneHead`. The same restore on WSL Linux (Android workload only) now gets past NETSDK1178. Waiting on the next push's run page | [view](#d-req-fn-057) |
...
| REQ-RAG-058 | F-LOCAL-LLM: one public provider over an internal `ILocalLlmRuntime` with one implementation per platform per `DECISIONS.md`; identical behaviour everywhere | Verified | 100% | 2026-09-25 verify: PASS — test REQ-RAG-058 SelectorReturnsOnnxGenAiHere | [view](#d-req-rag-058) |
...
| REQ-FN-060 | F-LOCAL-LLM: tokens per second, time to first token and peak memory recorded per platform on named devices in the support matrix | PARTIAL | 75% | [REQ-FN-060] 2026-09-25 Android phone recorded: owner's Galaxy S23 (Android 16), Qwen2.5 0.5B from the default Hugging Face address (six fingerprints matched), first token 0.06–0.10 s, 112–114 tokens/s, peak 703–763 MB, three runs, screenshots in `tests/.artifacts/probe/android-phone-20260925/`. Only the physical iPhone row remains (owner's Mac) | [view](#d-req-fn-060) |
