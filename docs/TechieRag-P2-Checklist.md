...
| REQ-FN-057 | F-PLATFORM: CI builds the probe for all four heads on every push and runs its button on an Android emulator where CI allows; an unbuildable head is reported, never skipped | Verified | 100% | 2026-09-25 verify: PASS — test REQ-FN-057 ProbeHeadNarrowsRestoreToOneHead | [view](#d-req-fn-057) |
...
| REQ-RAG-061 | F-LOCAL-LLM: one-time resumable download to the app-data root, `TECHIERAG_MODEL_BASE_URL`, SHA-256 verified, size known first, `ModelDownloadService` progress | Verified | 100% | 2026-09-25 verify: PASS — test REQ-RAG-061 PendingBytesSubtractFilesOnDiskAndPartRemainders | [view](#d-req-rag-061) |
...
| REQ-FN-060 | F-LOCAL-LLM: tokens per second, time to first token and peak memory recorded per platform on named devices in the support matrix | Verified | 100% | 2026-09-25 verify: PASS — test REQ-FN-060 LocalModelNumbersNameDeviceAndDate | [view](#d-req-fn-060) |
...
| REQ-FN-062 | F-LLM: per-vendor sign-in policy and flow (OpenAI, Anthropic, Google, Groq, xAI, Meta) checked against live documentation and recorded in `DECISIONS.md` before BRD-112 is built | Verified | 100% | 2026-09-25 verify: PASS — test REQ-FN-062 DecisionsRecordEveryVendorsSignInPolicy. Acceptance line corrected 2026-09-25 by *amend-docs (it had been copied from the ChatGPT sign-in row); the named test already checks `DECISIONS.md` per vendor | [view](#d-req-fn-062) |
...
| REQ-RAG-077 | Every outbound web fetch shall pass a connect-time SSRF guard (`HttpWebContentFetcher.CreateGuardedHandler`) that refuses private, loopback and link-local targe | Verified | 100% | 2026-09-25 verify: PASS — test REQ-RAG-077 RedirectEndingOnAPrivateAddressIsRefused | [view](#d-req-rag-077) |
...
| REQ-FN-070 | F-AUTODIST: the shipped AI reference and the two persona command files describe every phase-2 feature with one call example each | Verified | 100% | 2026-09-25 verify: PASS — test REQ-FN-070 DocsCopyIsIdenticalToShippedReference | [d](#d-req-fn-070) |
...
<a id="d-req-fn-070"></a>
- **REQ-FN-070** — F-AUTODIST: the shipped AI reference and the two persona command files describe every phase-2 feature with one call example each. *BRD:* BRD-167. *Phase 2.*
