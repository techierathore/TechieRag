...
|----|-------------|--------|---|---------|---------|
| REQ-RAG-015 | LIB: agentic retrieval contract in core — `TechieRag.Agentic` | Verified | 100% | 2026-09-25 verify: PASS — test REQ-RAG-015 RegisteredSearchReturnsRefsScoresAndStatus. ⚠ DevGuide 2026-09-25: the `weak` hint always quotes the cosine threshold even when reranking classified the result with the rerank threshold `src/TechieRag/Agentic/KnowledgeBaseTools.cs:218` | [view](#d-req-rag-015) |
| REQ-RAG-016 | LIB: `TechieRag.Agents` package — Microsoft Agent Framework agent over TechieRag, builder in the existing style, LM Studio primary | Verified | 100% | 2026-09-25 verify: PASS — test REQ-RAG-016 BuiltAgentAnswersFromDocuments. ⚠ DevGuide 2026-09-25: `Sources` fires on any tool result, not only a search, unlike its doc comment `src/TechieRag.Agents/TechieRagAgent.cs:77`; `TechieRagAgent.Retrieval` is public but not on `ITechieRagAgent` `TechieRagAgent.cs:34` | [view](#d-req-rag-016) |
...
| REQ-RAG-044 | The shipped `IVectorStore` set is SQLite, pgvector and Qdrant; `PgVectorStore` shall be proven against a real PostgreSQL through `LivePgVectorStoreTes | Verified | 100% | 2026-09-25 verify: not verified — every test named REQ-RAG-044 was skipped; acceptance not measured (2 test(s) skipped, none ran) | [view](#d-req-rag-044) |
| REQ-RAG-046 | Image generation, realtime audio, batch, fine-tuning, moderation and OCR endpoints are deferred and re-scoped on demand | Verified | 100% | 2026-09-25 verify: PASS — test REQ-RAG-046 NoPublicSurfaceClaimsADeferredEndpoint | [view](#d-req-rag-046) |
...
| REQ-RAG-053 | F-PLATFORM: model and cache root defaults to the per-user application data folder, host-overridable; embedded model, reranker and local LLM share it | Verified | 100% | 2026-09-25 verify: PASS — test REQ-RAG-053 AppleRootIsApplicationSupportNotDocuments. ⚠ DevGuide 2026-09-25: the comment still calls `TechieRag.Local` "the planned" package `src/TechieRag/Models/ModelRoot.cs:18` | [view](#d-req-rag-053) |
...
| REQ-FN-056 | F-PLATFORM: `samples/TechieRag.Probe`, a .NET MAUI app with Windows, Mac Catalyst, Android and iOS heads; one button embeds three texts, stores, searches, shows the top result and timings | Verified | 100% | 2026-09-25 verify: PASS — test REQ-FN-056 ProbeSupportsUnattendedAutoRun | [view](#d-req-fn-056) |
| REQ-FN-057 | F-PLATFORM: CI builds the probe for all four heads on every push and runs its button on an Android emulator where CI allows; an unbuildable head is reported, never skipped | Verified | 100% | 2026-09-25 verify: PASS — test REQ-FN-057 AppleJobsSelectTheXcodeTheirSdkAsksFor(job: "ios" | [view](#d-req-fn-057) |
...
| REQ-RAG-058 | F-LOCAL-LLM: one public provider over an internal `ILocalLlmRuntime` with one implementation per platform per `DECISIONS.md`; identical behaviour everywhere | Verified | 100% | 2026-09-25 verify: PASS — test REQ-RAG-058 EveryModelHasOneOnnxFileSet | [view](#d-req-rag-058) |
...
| REQ-RAG-061 | F-LOCAL-LLM: one-time resumable download to the app-data root, `TECHIERAG_MODEL_BASE_URL`, SHA-256 verified, size known first, `ModelDownloadService` progress | Verified | 100% | 2026-09-25 verify: PASS — test REQ-RAG-061 InterruptedDownloadResumes. ⚠ DevGuide 2026-09-25: `GetTerms()` reports the variant's full size while a resumed download transfers only the pending bytes, so a host showing it overstates the size `src/TechieRag.Local/LocalLlmProvider.cs:131` | [view](#d-req-rag-061) |
...
| REQ-FN-058 | F-LOCAL-LLM: `TechieRag.Local` ships buildTransitive targets with the native runtime libraries for all four platforms; no hand-written csproj changes | Verified | 100% | 2026-09-25 verify: PASS — test REQ-FN-058 TargetsWireCatalystAndCheckAndroid | [view](#d-req-fn-058) |
...
| REQ-FN-059 | F-LOCAL-LLM: the probe app's second button generates one sentence from the local model and shows time to first token and tokens per second | Verified | 100% | 2026-09-25 verify: PASS — test REQ-FN-059 ProbeSecondButtonShowsSentenceAndTimings. ⚠ DevGuide 2026-09-25: peak memory is labelled "MB" but computed in binary MiB, while the download line uses decimal MB, about 5 percent apart `samples/TechieRag.Probe/GenerationResult.cs:26` | [view](#d-req-fn-059) |
...
| REQ-FN-060 | F-LOCAL-LLM: tokens per second, time to first token and peak memory recorded per platform on named devices in the support matrix | Verified | 100% | 2026-09-25 verify: PASS — test REQ-FN-060 LocalModelNumbersNameDeviceAndDate. iOS is recorded from the simulator; the physical iPhone run stays open until the owner turns on Developer Mode and signs Xcode in (runbook step 1) | [view](#d-req-fn-060) |
...
| REQ-RAG-069 | F-LLM: subscription sign-in providers, one builder method per permitting vendor (first `UseChatGptSubscriptionLlm(signInCallback)`), host drives the browser, library yields an `ILlmProvider`, session persisted via a documented seam | Verified | 100% | 2026-09-25 verify: PASS — test REQ-RAG-069 PollWaitsUntilAuthorised. ⚠ DevGuide 2026-09-25: the `ILlmProvider` doc comment lists six implementations; the subscription provider is a seventh `src/TechieRag/Abstractions/ILlmProvider.cs:12` | [view](#d-req-rag-069) |
...
| REQ-RAG-078 | The system shall provide an `IDataConnector` framework with a `ConnectorRunner` that returns per-item results and per-item failure reasons and keeps incremental | Verified | 100% | 2026-09-25 verify: PASS — test REQ-RAG-078 RecordsItemFailureAndContinues | [view](#d-req-rag-078) |
...
| REQ-RAG-081 | A developer can ingest a mailbox over IMAP (generic, Gmail, Microsoft 365) or a local mbox file with folder, date, sender and subject filters, optional PDF/DOCX | Verified | 100% | 2026-09-25 verify: PASS — test REQ-RAG-081 AsksOnlyForMailSinceThePreviousRun | [view](#d-req-rag-081) |
| REQ-RAG-082 | Connector transports shall reuse the SSRF-guarded connect-time handler, cap a run at `MaxTotalBytes` (64 MB), and the IMAP client shall refuse command injection | Verified | 100% | 2026-09-25 verify: PASS — test REQ-RAG-082 StopsAtMaxTotalBytes. ⚠ DevGuide 2026-09-25: the IMAP connection opens a plain socket with no private-address check, unlike the HTTP transport; likely intended for LAN mail servers but undocumented `src/TechieRag/Connectors/Email/SocketImapConnection.cs:72` | [view](#d-req-rag-082) |
...
| REQ-RAG-085 | The system shall expose an `IMcpServerRegistry` with an in-memory implementation, a `McpTrustPolicy`, and the tool-to-server mapping (`McpToolHandler.ServerName | Verified | 100% | 2026-09-25 verify: PASS — test REQ-RAG-085 PolicyViolatingConfigurationIsRefusedAtRegistrat | [view](#d-req-rag-085) |
...
| REQ-RAG-092 | The system shall provide workspace primitives (`IWorkspaceStore`, `WorkspaceManager`, `Workspace`): isolated documents and settings, document pinning, a per-wor | Verified | 100% | 2026-09-25 verify: PASS — test REQ-RAG-092 StreamedSourcesExcludeOtherWorkspaceDocuments | [view](#d-req-rag-092) |
...
| REQ-FN-066 | `AddTechieRag(IConfiguration)` and `AddTechieRag(TechieRagConfig)` shall map every configuration field the builder accepts, including `VectorStore.ApiKey`, embe | Verified | 100% | 2026-09-25 verify: PASS — test REQ-FN-066 AppSettingsApiKeyAndSystemPromptReachBuiltInstanc. ⚠ DevGuide 2026-09-25: `Llm.Source = Subscription` from configuration throws the generic "Unsupported LLM source" instead of the `SubscriptionNeedsSignIn` message the factory gives `src/TechieRag/TechieRagBuilder.cs:851` | [view](#d-req-fn-066) |
