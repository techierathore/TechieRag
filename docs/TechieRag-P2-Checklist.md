...
|----|-------------|--------|---|---------|---------|
| REQ-RAG-015 | LIB: agentic retrieval contract in core — `TechieRag.Agentic` | Not Started | 0% | Not started. Design: `docs/TechieRag.Agents-Proposal.md` §3. Migrated 2026-09-24, status preserved. | [view](#d-req-rag-015) |
| REQ-RAG-016 | LIB: `TechieRag.Agents` package — Microsoft Agent Framework agent over TechieRag, builder in the existing style, LM Studio primary | Not Started | 0% | Not started; depends on REQ-RAG-015. Design: `docs/TechieRag.Agents-Proposal.md` §4–5. Migrated 2026-09-24, status preserved. | [view](#d-req-rag-016) |
| REQ-RAG-017 | LIB: public seam adapters in `TechieRag.Agents` — `ILlmProvider`→`IChatClient`, `IToolHandler`↔`AITool`, middleware→`IProgress<AgentStep>`, `IConversationMemory`→`ChatHistoryProvider` | Not Started | 0% | Not started; depends on REQ-RAG-016. Migrated 2026-09-24, status preserved. | [view](#d-req-rag-017) |
| REQ-FN-004 | Consumer-facing install documentation shall lead with the public feed (`dotnet add package TechieRag` / `TechieRag.Embedded`, no authentication, no PA | Done (pre-existing) | 100% | Verified 2026-09-03: README, user guides, AI reference and command files lead with nuget.org; proven from a fresh project with `<clear/>`. Given BRD-163 on 2026-09-24. History: `docs/OldDocs/TechieRag-Checklist.md`. | [view](#d-req-fn-004) |
| REQ-FN-005 | Pack + publish `TechieRag.Agents` alongside the other two packages on both feeds at the same version | Not Started | 0% | Not started; one `dotnet pack` line in each workflow once REQ-RAG-016 exists. Migrated 2026-09-24, status preserved. | [view](#d-req-fn-005) |
| REQ-FN-006 | Repository separation — the app (now **Sevak**, TechieDesk renamed) leaves this repo; solution = `src/*` + `tests/TechieRag.Tests` | Implemented | 75% | 2026-09-24: Sevak repository green (Sevak#REQ-FN-054 Verified); `apps/`, app tests, harnesses, app docs, `publish-desktop.yml` deleted here; `TechieRag.slnx` trimmed; library tests PASS via ladder; docs rewritten by day-1. Awaiting `*verify`. | [view](#d-req-fn-006) |
| REQ-RAG-042 | The system shall run declarative agent flows: five node kinds (Agent, Tool, Condition, Handoff, Terminal), `FlowCondition` routing as data with 11 ope | Implemented | 75% | Built 2026-08-01 in `src/TechieRag/Orchestration/` (22 files, zero packages); was Implemented 95%. Open clause: a live blocked Tool-node run on a screen (Sevak). Migrated 2026-09-24. History: `docs/OldDocs/TechieRag-Checklist.md`. | [view](#d-req-rag-042) |
| REQ-RAG-044 | The shipped `IVectorStore` set is SQLite, pgvector and Qdrant; `PgVectorStore` shall be proven against a real PostgreSQL through `LivePgVectorStoreTes | Implemented | 75% | 7 hermetic tests pass; `LivePgVectorStoreTests` (5) skip until `TechieRagTestPostgres` names a server; was Implemented 85%. Migrated 2026-09-24. History: `docs/OldDocs/TechieRag-Checklist.md`. | [view](#d-req-rag-044) |
| REQ-RAG-046 | Image generation, realtime audio, batch, fine-tuning, moderation and OCR endpoints are deferred and re-scoped on demand | Not Started | 0% | Deferred; grep of `src/` for image generation, batch, moderation, fine-tuning returns nothing (2026-07-29). Migrated 2026-09-24. | [view](#d-req-rag-046) |
| REQ-RAG-050 | Every user-visible message the flow engine produces (refusals, budget exhaustion, validation) shall be a `FlowMessage` carrying a `FlowMessageCodes` c | Implemented | 75% | 22 literal sites replaced by `FlowMessage` codes with arguments (2026-08-02); was Implemented 80%; app-side rendering is Sevak's. Migrated 2026-09-24. History: `docs/OldDocs/TechieRag-Checklist.md`. | [view](#d-req-rag-050) |
| REQ-RAG-051 | `AgentToolHandler.ForFlow` shall report a blocked or budget-exhausted inner run as an unsuccessful `ToolResult` (`IsSuccess = false`), never as a succ | Implemented | 75% | 11/11 tests prove `IsSuccess = false` on both failure modes; was Implemented 95%; the on-screen clause is Sevak's. Migrated 2026-09-24. History: `docs/OldDocs/TechieRag-Checklist.md`. | [view](#d-req-rag-051) |
| REQ-RAG-052 | `EmbeddedEmbeddingProvider` shall encode text as XLM-RoBERTa expects (SentencePiece ids shifted by the fairseq offset, wrapped in `<s>` … `</s>`), eve | Implemented | 75% | Fairseq offset and `<s>…</s>` wrapping fixed; `EmbeddingSignature` stamps vectors; was Implemented 95%. Migrated 2026-09-24. History: `docs/OldDocs/TechieRag-Checklist.md`. | [view](#d-req-rag-052) |
| REQ-FN-054 | F-PLATFORM: platform support matrix in BRD §9 and the UsageGuide, per package × platform, tested cells name a device and date | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-fn-054) |
| REQ-RAG-053 | F-PLATFORM: model and cache root defaults to the per-user application data folder, host-overridable; embedded model, reranker and local LLM share it | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-rag-053) |
| REQ-FN-055 | F-PLATFORM: `TechieRag.Embedded` ships buildTransitive targets with the native ONNX Runtime wiring for Mac Catalyst, iOS and Android; Sevak removes its hand-written copy | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-fn-055) |
| REQ-RAG-054 | F-PLATFORM: on Android and iOS `UseEmbedded()` defaults to a 384-dimension model and refuses bge-m3 with a message naming its 2.3 GB size | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-rag-054) |
| REQ-RAG-055 | F-PLATFORM: every model download reports its total size before the first byte and progress through `ModelDownloadService` events | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-rag-055) |
| REQ-RAG-056 | F-PLATFORM: `SqliteVecStore` managed similarity search vectorised and documented as the supported path; dead sqlite-vec code removed or marked; 1k/10k/50k timings recorded | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-rag-056) |
| REQ-FN-056 | F-PLATFORM: `samples/TechieRag.Probe`, a .NET MAUI app with Windows, Mac Catalyst, Android and iOS heads; one button embeds three texts, stores, searches, shows the top result and timings | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-fn-056) |
| REQ-FN-057 | F-PLATFORM: CI builds the probe for all four heads on every push and runs its button on an Android emulator where CI allows; an unbuildable head is reported, never skipped | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-fn-057) |
| REQ-RAG-057 | F-LOCAL-LLM: `TechieRag.Local` package with `.UseLocalLlm()` via `UseCustomLlmProvider`, platform-appropriate default model, overloads for a model id or folder; nothing in core | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-rag-057) |
| REQ-RAG-058 | F-LOCAL-LLM: one public provider over an internal `ILocalLlmRuntime` with one implementation per platform per `DECISIONS.md`; identical behaviour everywhere | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-rag-058) |
| REQ-RAG-059 | F-LOCAL-LLM: the provider fully implements `ILlmProvider` including typed streaming, applies the chat template, honours every option and cancellation, refuses an over-long prompt | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-rag-059) |
| REQ-RAG-060 | F-LOCAL-LLM: memory check before load; refuses with a clear message instead of letting the OS kill the app | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-rag-060) |
| REQ-RAG-061 | F-LOCAL-LLM: one-time resumable download to the app-data root, `TECHIERAG_MODEL_BASE_URL`, SHA-256 verified, size known first, `ModelDownloadService` progress | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-rag-061) |
| REQ-RAG-062 | F-LOCAL-LLM: download requires explicit acceptance of the model's terms; weights never inside the package or the app bundle | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-rag-062) |
| REQ-FN-058 | F-LOCAL-LLM: `TechieRag.Local` ships buildTransitive targets with the native runtime libraries for all four platforms; no hand-written csproj changes | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-fn-058) |
| REQ-RAG-063 | F-LOCAL-LLM: `CompleteAsync<T>` and JSON mode return valid JSON for the schema; `SupportsToolCalling` false until a test proves otherwise | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-rag-063) |
| REQ-RAG-064 | F-LOCAL-LLM: `LlmSource.Local`, a `LlmProviderFactory` arm and a `LlmConnectorCatalog` row so `ModelRouter` resolves `local/<model>`; no endpoint, no key | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-rag-064) |
| REQ-RAG-065 | F-LOCAL-LLM: `EstimateTokenCount` uses the model's real tokenizer | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-rag-065) |
| REQ-FN-059 | F-LOCAL-LLM: the probe app's second button generates one sentence from the local model and shows time to first token and tokens per second | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-fn-059) |
| REQ-RAG-066 | F-LOCAL-LLM: live tests gated by `LiveLocalLlmFactAttribute` in a non-parallel collection, skipping with a reason when no model is present | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-rag-066) |
| REQ-FN-060 | F-LOCAL-LLM: tokens per second, time to first token and peak memory recorded per platform on named devices in the support matrix | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-fn-060) |
| REQ-FN-061 | F-LOCAL-LLM: UsageGuide gains a `TechieRag.Local` section; matrix updated; every 'offline' / 'air-gapped' claim corrected to 'downloads once, then works offline' | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-fn-061) |
| REQ-RAG-067 | F-LLM: additive typed streaming method on `ILlmProvider` (text delta, tool call, completed-with-usage); `ChatStreamAsync` unchanged and implemented over it; all six providers | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-rag-067) |
| REQ-RAG-068 | F-AGENT: `AgentLoopRunner` streaming run over typed events, tools executed through `ToolRegistry`; `TechieRag.Agents` `IChatClient` adapter streams the same | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-rag-068) |
| REQ-RAG-069 | F-LLM: subscription sign-in providers, one builder method per permitting vendor (first `UseChatGptSubscriptionLlm(signInCallback)`), host drives the browser, library yields an `ILlmProvider`, session persisted via a documented seam | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-rag-069) |
| REQ-FN-062 | F-LLM: per-vendor sign-in policy and flow (OpenAI, Anthropic, Google, Groq, xAI, Meta) checked against live documentation and recorded in `DECISIONS.md` before BRD-112 is built | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-fn-062) |
| REQ-RAG-070 | F-LLM: `LlmSource.Subscription`, factory arm, catalog rows per vendor carrying stated terms and the date checked; `ModelRouter` resolves a subscription model; a vendor with no permitted flow has no method | Not Started | 0% | Added 2026-09-24 by `*amend-docs` (owner decisions in `docs/TechieRag-Update-Brief.md`; plan 08). | [view](#d-req-rag-070) |
| REQ-RAG-071 | The system shall offer pluggable chunking through `IChunker` with recursive, token, markdown/code-aware and sentence strategies (`WithChunking`, `WithCustomChun | Done (pre-existing) | 100% | TechieDesk REQ-RAG-026 Verified; four strategies in `src/TechieRag/Processors/Chunking/`. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-071) |
| REQ-RAG-072 | The system shall extract text from XLSX, PPTX and CSV files through dedicated processors | Done (pre-existing) | 100% | TechieDesk REQ-RAG-033 Verified; `XlsxProcessor`, `PptxProcessor`, `CsvProcessor`. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-072) |
| REQ-RAG-073 | The system shall ingest audio files through an `AudioTranscriptionProcessor` that transcribes via an OpenAI-compatible speech endpoint; a local Whisper ONNX pat | Implemented | 75% | TechieDesk REQ-RAG-040 Verified 90%: API path proven, local Whisper absent. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-073) |
| REQ-RAG-074 | A developer can ingest a single web page by URL: fetch, clean to text, chunk and embed (`WebIngestionExtensions`, `WebPageReader`) | Done (pre-existing) | 100% | TechieDesk REQ-RAG-016 Verified. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-074) |
| REQ-RAG-075 | A developer can crawl a site with depth and maximum-link limits (`SiteCrawler`, `WebCrawlOptions`) and ingest every page reached | Done (pre-existing) | 100% | TechieDesk REQ-RAG-017 Verified. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-075) |
| REQ-RAG-076 | A developer can ingest a YouTube video's transcript by URL (`YouTubeTranscriptReader`); blocked today because YouTube's timed-text endpoint returns empty bodies | Blocked | 25% | TechieDesk REQ-RAG-018 Blocked 40%: YouTube timed-text returns empty bodies; owner decides the approach (`docs/Sevak-TechieRag-Feedback.md`). Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-076) |
| REQ-RAG-077 | Every outbound web fetch shall pass a connect-time SSRF guard (`HttpWebContentFetcher.CreateGuardedHandler`) that refuses private, loopback and link-local targe | Done (pre-existing) | 100% | Connect-time SSRF guard in `HttpWebContentFetcher.CreateGuardedHandler` (REQ-RAG-031 remarks, 2026-08). Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-077) |
| REQ-RAG-078 | The system shall provide an `IDataConnector` framework with a `ConnectorRunner` that returns per-item results and per-item failure reasons and keeps incremental | Done (pre-existing) | 100% | TechieDesk REQ-RAG-032 Verified; runner results and sync state in `src/TechieRag/Connectors/`. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-078) |
| REQ-RAG-079 | A developer can connect a GitHub or GitLab repository and ingest its files with branch and glob filters (`RepositoryConnector`) | Done (pre-existing) | 100% | TechieDesk REQ-RAG-019 Verified. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-079) |
| REQ-RAG-080 | A developer can connect a Confluence space and ingest its pages, with scheme, host and port pinned on cursors and links so the token never reaches a host named  | Done (pre-existing) | 100% | TechieDesk REQ-RAG-020 Verified 95%; TR-RAG-018 host pinning fixed. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-080) |
| REQ-RAG-081 | A developer can ingest a mailbox over IMAP (generic, Gmail, Microsoft 365) or a local mbox file with folder, date, sender and subject filters, optional PDF/DOCX | Implemented | 75% | TechieDesk REQ-RAG-049 Implemented 90%: only the mbox path exercised; no real IMAP server contacted. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-081) |
| REQ-RAG-082 | Connector transports shall reuse the SSRF-guarded connect-time handler, cap a run at `MaxTotalBytes` (64 MB), and the IMAP client shall refuse command injection | Done (pre-existing) | 100% | TR-RAG-017/019/027…030 fixed in `HttpConnectorTransport`, `SocketImapConnection`, `ImapByteReader`. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-082) |
| REQ-RAG-083 | `ConnectorRunner` shall hand each fetched document to ingestion as it arrives instead of collecting every document before ingesting, so a run stopped part-way k | Not Started | 0% | TR-RAG-020…022 open: runner collects all documents before ingesting; transport is GET-only. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-083) |
| REQ-RAG-084 | The system shall consume Model Context Protocol tool servers in the agent loop through `McpClient` with stdio and HTTP transports and `McpToolHandler` | Done (pre-existing) | 100% | TechieDesk REQ-RAG-038 Verified. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-084) |
| REQ-RAG-085 | The system shall expose an `IMcpServerRegistry` with an in-memory implementation, a `McpTrustPolicy`, and the tool-to-server mapping (`McpToolHandler.ServerName | Implemented | 75% | TechieDesk REQ-RAG-023 Implemented 90%; TR-RAG-041 tool-to-server mapping fixed in the library; persistence of servers is Sevak's. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-085) |
| REQ-RAG-086 | `IFlowGuardrail` shall run at Input, Output and ToolCall, deny by default, and `FlowRuntime.HostGuardrails` shall be set only by the host so a flow definition c | Implemented | 75% | TechieDesk REQ-NFR-013 Implemented 95%: host guardrail as `DelegateFlowGuardrail`; live blocked call on a screen is Sevak's. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-086) |
| REQ-RAG-087 | A flow definition shall round-trip through `FlowSerializer` and `FlowValidator` shall refuse an invalid graph (unreachable nodes, cycles without `AllowCycles`,  | Implemented | 75% | Built with REQ-RAG-042 (`FlowSerializer`, `FlowValidator`, `FlowValidationCodes`); editor round-trip proof is Sevak's REQ-UI-040. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-087) |
| REQ-RAG-088 | A flow run shall complete or fail within a configurable time limit; a Tool-node flow that waits on a host confirmation shall end with a coded timeout message in | In Progress | 25% | TechieDesk REQ-FN-053 In Progress 40%: time limit added to flow runs; not reproduced at the service layer. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-088) |
| REQ-RAG-089 | The system shall provide database-backed conversation memory with threads (`DbConversationMemory`, `IConversationStore`) on SQLite and PostgreSQL | Done (pre-existing) | 100% | TechieDesk REQ-RAG-027 Verified; PostgreSQL store untested live. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-089) |
| REQ-RAG-090 | `IConversationStore` shall persist every message of a thread (user, assistant, sources as JSON, content parts) per workspace and user | Done (pre-existing) | 100% | TechieDesk REQ-RAG-008 Verified. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-090) |
| REQ-RAG-091 | The system shall build the model context from persisted history with token-aware trimming that keeps the system message and the most recent turns | Done (pre-existing) | 100% | TechieDesk REQ-RAG-009 Verified. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-091) |
| REQ-RAG-092 | The system shall provide workspace primitives (`IWorkspaceStore`, `WorkspaceManager`, `Workspace`): isolated documents and settings, document pinning, a per-wor | Done (pre-existing) | 100% | TechieDesk REQ-RAG-028/007/013/014/015 Verified. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-092) |
| REQ-RAG-093 | Ingestion shall deduplicate by content hash so a document embedded once is reused across workspaces (`IWorkspaceStore.FindDocumentIdByHashAsync`) | Done (pre-existing) | 100% | TechieDesk REQ-RAG-012 Verified. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-093) |
| REQ-RAG-094 | Context assembly shall signal truncation (`WorkspaceContext.WasTruncated`, the `ContextTruncated` event) and evict retrieved chunks before pinned ones | Done (pre-existing) | 100% | TechieDesk REQ-RAG-048 Verified. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-094) |
| REQ-RAG-095 | Deleting a document shall remove its vectors and its membership from every workspace; `IWorkspaceStore.RemoveDocumentAsync` deletes only the membership row toda | Needs re-verify | 50% | TechieDesk REQ-FN-012 Needs re-verify 60%; TR-RAG-004 open: `RemoveDocumentAsync` deletes only the membership row. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-095) |
| REQ-RAG-096 | `PromptTemplateEngine.FormatContext` shall signal truncation the same way `WorkspaceManager` does instead of cutting context silently (TR-RAG-009) | Not Started | 0% | TR-RAG-009 open. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-096) |
| REQ-RAG-097 | The system shall provide an `IReranker` stage with a local ONNX cross-encoder (`OnnxCrossEncoderReranker`, bge-reranker-v2-m3, in `TechieRag.Embedded`) and API  | Done (pre-existing) | 100% | TechieDesk REQ-RAG-025 Verified; TR-RAG-044/045 fixed. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-097) |
| REQ-RAG-098 | A developer can switch reranking per call through `SearchOptions.Rerank` and set the default with `WithRerankEnabledByDefault`; the workspace flag is authoritat | Done (pre-existing) | 100% | TechieDesk REQ-RAG-047 Verified. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-098) |
| REQ-RAG-099 | A developer can pick a hosted model by name alone: `ModelRouter` resolves the longest unambiguous prefix in `LlmConnectorCatalog` (OpenAI, Anthropic, Gemini, Gr | Done (pre-existing) | 100% | TechieDesk REQ-RAG-034 Verified 95%. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-099) |
| REQ-RAG-100 | A developer can use Cohere, Voyage, Mistral and Google Gemini embedding providers (`UseCohereEmbedding`, `UseVoyageEmbedding`, `UseMistralEmbedding`, `UseGemini | Done (pre-existing) | 100% | TechieDesk REQ-RAG-035 Verified 95%: wire contract only; Voyage and Mistral run through the OpenAI-compatible provider. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-100) |
| REQ-RAG-101 | A developer can send images with a chat message through `IMultimodalLlmProvider`, `ChatImage` and `ChatContentPart`, mapped to each vendor's wire shape; audio a | Done (pre-existing) | 100% | TechieDesk REQ-RAG-039 Verified 90%: images only. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-101) |
| REQ-RAG-102 | The system shall pass prompt-caching hints through to Anthropic and Gemini (`PromptCacheOptions`) | Done (pre-existing) | 100% | TechieDesk REQ-RAG-043 Verified. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-102) |
| REQ-RAG-103 | Streaming RAG shall return source citations alongside the text (`RagStreamEvent`, `AskStreamWithSourcesAsync`, `ChatWithRagStreamWithSourcesAsync`) and honour ` | Done (pre-existing) | 100% | TechieDesk REQ-RAG-024 Verified (closed TR-RAG-001). Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-103) |
| REQ-RAG-104 | Cost shall come from a configurable pricing table (`WithModelPricing`, `ModelPricing`) and streamed-token usage shall be reported correctly on every provider | Done (pre-existing) | 100% | TechieDesk REQ-RAG-029 Verified (closed TR-RAG-002). Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-104) |
| REQ-RAG-105 | The system shall provide `ISpeechToText` and `ITextToSpeech` abstractions with OpenAI-compatible providers (`UseSpeechToText`) | Done (pre-existing) | 100% | TechieDesk REQ-RAG-041 Verified 90%: no live speech endpoint. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-105) |
| REQ-FN-063 | `TechieRag` and `TechieRag.Telemetry` shall target `net10.0` and `net8.0`; `TechieRag.Embedded` targets `net10.0` because ONNX Runtime requires it | Done (pre-existing) | 100% | TechieDesk REQ-RAG-037 Verified; tests run on net10.0 only (Architecture open question 8). Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-fn-063) |
| REQ-FN-064 | `TechieRag.Telemetry` shall be a separate opt-in package: OTLP or console exporters, tracing and metrics off by default, a loopback endpoint by default and a no | Done (pre-existing) | 100% | TechieDesk REQ-RAG-036 and REQ-NFR-008 Verified. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-fn-064) |
| REQ-FN-065 | Both publishing workflows shall pack and publish `TechieRag.Telemetry` with the same SourceLink, symbol and README settings as the other packages; today no work | Not Started | 0% | Found by the 2026-09-24 code scan: neither workflow packs `TechieRag.Telemetry`; its csproj lacks SourceLink and symbol settings. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-fn-065) |
| REQ-RAG-106 | `TechieRagBuilder.Build()` shall pass the embedding provider's `Dimensions` into every vector store instead of the 1024 default, so Cohere, OpenAI and Gemini em | Not Started | 0% | Found by the 2026-09-24 code scan: `TechieRagBuilder.cs:854-865` constructs every store without dimensions. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-rag-106) |
| REQ-FN-066 | `AddTechieRag(IConfiguration)` and `AddTechieRag(TechieRagConfig)` shall map every configuration field the builder accepts, including `VectorStore.ApiKey`, embe | Not Started | 0% | Found by the 2026-09-24 code scan: `ServiceCollectionExtensions.cs:133-272`. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-fn-066) |
| REQ-FN-067 | Exactly one path shall publish to nuget.org: the manual dispatch of `publish-nuget.yml`; the automatic `publish-nuget-org` job in `publish-github-packages.yml`  | Not Started | 0% | Found by the 2026-09-24 code scan: `publish-github-packages.yml` job `publish-nuget-org`. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-fn-067) |
| REQ-FN-068 | Unit tests shall cover processors, providers, the agent loop, memory and cost math; live tests are gated by attributes that skip with a reason | Done (pre-existing) | 100% | TechieDesk REQ-RAG-030 Verified; 634 test methods, PASS 2026-09-24 via the build ladder. Added 2026-09-24 by day-1 brownfield (harvested). | [view](#d-req-fn-068) |
...
- **REQ-RAG-076** — A developer can ingest a YouTube video's transcript by URL (`YouTubeTranscriptReader`); blocked today because YouTube's timed-text endpoint returns empty bodies. *BRD:* BRD-120. *Phase 2.*
