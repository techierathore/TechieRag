...
| Q6 | Tests | xUnit 2.9.3 with coverlet, `tests/TechieRag.Tests` (96 files, about 634 test methods; 558 `[Fact]`, 35 `[Theory]`, 41 live-gated). Live tests are gated by custom `FactAttribute`s that set `Skip` with a reason. No mocking library; hand-written doubles in `TestDoubles/`. A second test project, `TechieRag.Agents.Tests`, is planned with the Agents package (BRD-84). | code; answer set (xUnit) |
...
| `TechieRag.Agents` *(planned, BRD-84/85, ADR-008)* | class library, NuGet package | Agents on Microsoft Agent Framework over TechieRag: fluent builder, agentic retrieval context provider, public seam adapters. |
| `TechieRag.Local` *(planned, BRD-96…109, ADR-014)* | class library, NuGet package | In-process local language model behind `UseLocalLlm()`; one internal runtime interface, one implementation per platform; `buildTransitive` native wiring. |
| `TechieRag.Local.Tests`, `TechieRag.Agents.Tests` *(planned)* | xUnit test projects | Conformance and live-gated tests for the two new packages. |
...
flowchart TB
  subgraph Packages["src — the packages"]
    ...
    Agents["TechieRag.Agents — planned (BRD-84/85)"]
    Local["TechieRag.Local — planned (BRD-96…109)"]
  end
...
flowchart TB
  ...
  subgraph Impl["Implementations"]
    ...
    Web["Web — HttpWebContentFetcher, SiteCrawler, WebPageReader, YouTubeTranscriptReader"]
...
| Model caches (files) | `<TechieRag.Embedded.dll folder>/models/bge-m3/`, `.../models/bge-reranker-v2-m3/`; about 2.3 GB for bge-m3 | Read-only on iOS and Android; moves to the per-user application data root under BRD-89. The live tests expect `~/.cache/techierag-models/` instead (Open question 7). |
...
| 2026-09-03 | ADR-008: `TechieRag.Agents` on Microsoft Agent Framework as a sibling package; nothing MAF-shaped enters core | MAF's ecosystem is worth adopting, its monthly cadence is not worth importing into every consumer (BRD-84) | planned |
| 2026-09-03 | ADR-009: the agentic retrieval contract lives in core (`TechieRag.Agentic`) and both loops bind it (BRD-83) | One tested contract instead of two prompts drifting apart | planned |
...
| 2026-09-03 | ADR-011: seam adapters are public API of `TechieRag.Agents` (BRD-85) | A consumer keeps one provider configuration and one tool catalogue | planned |
...
| 2026-09-24 | ADR-013: typed streaming events on `ILlmProvider`, additive, before `TechieRag.Local` exists (BRD-110, BRD-111) | Chatur TR-RAG-002; six providers once instead of seven twice; keeps the 1.0.x line | planned |
| 2026-09-24 | ADR-014: `TechieRag.Local` as a sibling package; one public provider over an internal per-platform runtime; weights downloaded, never packed (BRD-96…109) | MyDiary TR-RAG-001; neither LLamaSharp nor ONNX Runtime GenAI is proven on all four platforms, so the design must not bet on one | planned |
...
| `Web` | URL fetch, site crawl with depth and link caps, page reading, YouTube transcripts, `WebIngestionExtensions` | HtmlAgilityPack, `HttpClient` |
...
1. **sqlite-vec is never loaded.** `SqliteVecStore.TryLoadSqliteVecExtension` always returns false and the `LoadExtension("vec0")` call is commented out (`SqliteVecStore.cs:118-131`); every search is a managed full scan, and the class's XML docs still say fallback "returns empty results". BRD-93 makes the managed path the documented, vectorised, measured one and removes the dead code.
...
10. **Download progress is written with `Console.WriteLine`** in `EmbeddedEmbeddingProvider` (lines 305-315) instead of the `ModelDownloadService` event; BRD-92 generalises the service.
11. **Local runtime choice** (LLamaSharp vs ONNX Runtime GenAI, per platform) is decided after the probe comparison on named devices (plan 08 step 2) and recorded in `DECISIONS.md` before BRD-96…98 are built. Costly to reverse.
