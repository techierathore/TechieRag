...
| Installed agent files | `build/TechieRag.targets`, `build/content/` (installed as `.techierag/TechieRag-AI-Reference.md`, `.claude/commands/techierag.md`, `.opencode/command/techierag.md`) | Consuming developer's AI agent | — (library, no mockup) | AI reference, two persona command files, one call example per feature |
...
- **BRD-88** — The BRD and the UsageGuide shall carry a platform support matrix stating, per package (`TechieRag`, `TechieRag.Embedded`, `TechieRag.Agents`, `TechieRag.Local`) and per platform (Windows, macOS / Mac Catalyst, Android, iOS), whether it is supported, tested on a named device, or not supported. The matrix lives in the phase-1 BRD's "Constraints and assumptions" section and in the UsageGuide's Platform notes, and the two are changed together. "Supported" means the package is built for that platform and no run is recorded there yet; "tested" is written only from a recorded probe run and names the device and the date; "not supported" means the package cannot run there *(F-PLATFORM; wording corrected 2026-09-25)* *Screen:* Platform groundwork
...
| Surface: Packaging and quality follow-ups | 9 | 9 | 0 | Done |
