...
- The in-process language model (`TechieRag.Local`, ONNX Runtime GenAI) has no default download address for the phone model (Qwen2.5 0.5B) until the owner's mirror is published: set `TECHIERAG_MODEL_BASE_URL`, or the first load names that variable and downloads nothing. An Intel Mac has no engine (the package ships none) and throws `PlatformNotSupportedException`. Subscription sign-in: REQ-RAG-069 and REQ-RAG-070. Typed streaming (REQ-RAG-067) and the streaming agent loop (REQ-RAG-068) exist since 2026-09-24.
...
Peak memory: Windows, the peak working set; macOS plain process, the peak resident set (`/usr/bin/time -l`); Mac Catalyst and the iOS simulator, the peak physical footprint the probe shows, which matches `footprint <pid>` and is what the iOS memory limit counts. Time to first token includes reading the prompt; tokens per second counts the tokens after the first. The Catalyst range spans a run just after the download and a fresh launch. Raw lines and screenshots: `tests/.artifacts/probe/maccatalyst/generate-phi3-*`, `tests/.artifacts/probe/ios/generate-qwen*`; plain-process JSON: `tests/.artifacts/local-llm-bench/`.

### Local model: engine comparison on the Windows laptop (2026-09-24)

Both engines directly on the Windows 11 laptop (Core i5-11300H, 16 GB; WSL gets 7.6 GB), before `TechieRag.Local` had one; with the M4 Max repeat it is what the owner chose from (`DECISIONS.md` 2026-09-25). Same prompt, greedy, 128-token answer, three runs per row; a shared host, so the better runs are the fairer ones. Raw JSON: `tests/.artifacts/local-llm-bench/`.
...
- **Model root (REQ-RAG-053).** Every model (embedding model, reranker, later the local model) is downloaded to `<LocalApplicationData>/TechieRag/models/<model>`: `%LOCALAPPDATA%\TechieRag\models` on Windows; `~/Library/Application Support/TechieRag/models` on Mac Catalyst (inside the app's container when it is sandboxed) and plain macOS; the app's private `files` folder on Android (`/data/user/0/<package>/files/TechieRag/models`); `<app container>/Library/Application Support/TechieRag/models` on iOS. On iOS and Mac Catalyst .NET reports `Documents` as `LocalApplicationData`, so TechieRag uses `<home>/Library/Application Support` there instead: model files never land in the user's Documents (proved 2026-09-25 by the probe on Mac Catalyst, macOS 27, and the iOS 26.1 simulator). Move it with `builder.UseModelRoot(path)` (or `ModelRoot.Set(path)`, or the `TECHIERAG_MODEL_ROOT` environment variable) before the first model loads. A complete copy an older version left next to the assembly is still used, so upgrading does not download again.
...
| Mac Catalyst | owner's Mac | — | — | — | — | not run yet |
...
4. Press **Generate one sentence** (AutomationId `RunGenerateButton`, REQ-FN-059) the same way, accept the model's terms in the dialog, and record the time to first token, tokens per second and peak memory from `GenerationTimingsLabel` in that platform's row of "Local model: measured per platform" above (device, date, model), and set the `TechieRag.Local` cell to `tested (<device>, <date>)`. Phones and the iPhone download Qwen2.5 0.5B, whose files have no default address yet: until the owner's mirror is published, serve `tests/.artifacts/model-mirror/` over HTTP on the Mac and start the app with `TECHIERAG_MODEL_BASE_URL` pointing at it.
