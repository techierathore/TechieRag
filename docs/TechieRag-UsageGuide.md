...
- **Expected:** 1) text deltas, then one `Completed` with usage from the model's own tokenizer; the DTO parses; 2) `LocalPromptTooLongException` naming both lengths, nothing generated; 3) `LocalModelMemoryException` naming the shortfall, the app keeps running. **Today** the first load throws `PlatformNotSupportedException` until the owner chooses the runtime per platform (`docs/TechieRag-Decision-Request.md`); the conformance suite covers these against a scripted runtime.
...
- Mac Catalyst and iOS heads of the probe have not been built or run yet: they need a Mac (the owner's runbook below). Their native ONNX wiring (`TechieRag.Embedded.targets`) is unproven until then.
- No in-process language model runs yet: `TechieRag.Local` is built up to its runtime, which waits for the owner's per-platform choice (REQ-RAG-058, `docs/TechieRag-Decision-Request.md`); until then a load throws `PlatformNotSupportedException`. Subscription sign-in: REQ-RAG-069 and REQ-RAG-070. Typed streaming (REQ-RAG-067) and the streaming agent loop (REQ-RAG-068) exist since 2026-09-24.
...
3. Built up to its inference runtime on 2026-09-24 (REQ-RAG-057 to REQ-RAG-066); every platform stays not supported until the owner chooses the runtime per platform (`docs/TechieRag-Decision-Request.md`). Shipped by the engines today (nuget.org, 2026-09-24): ONNX Runtime GenAI 0.16.0 for Windows, Android and iOS, not Mac Catalyst; LLamaSharp 0.27.0 for Windows and Android, not iOS or Mac Catalyst. Engine numbers measured on the Windows laptop are under "Local model: runtime comparison" below; the probe's second button (REQ-FN-059) records the per-device numbers.

### Local model: runtime comparison (REQ-FN-060, BRD-108)

Measured 2026-09-24 on the Windows 11 laptop (Mi NoteBook Pro, Core i5-11300H, 4 cores / 8 threads, 16 GB; WSL gets 7.6 GB) with the two candidate engines directly (harness and raw JSON: `tests/.artifacts/local-llm-bench/`), not yet through `TechieRag.Local`, which has no engine until the owner decides. Same templated prompt for both engines, greedy, a 128-token answer, three runs per row, each engine in its own process; the host was shared with other builds, so the ranges are wide and the better runs are the fairer ones. Time to first token includes reading the prompt; peak memory is the process peak.
...
| Android phone, iPhone, Mac | — | — | — | — | — | owner's devices, not run |

Token counts of three fixed sentences agree exactly between the two engines for both models (Qwen2.5 10 / 13 / 12, Phi-3 12 / 14 / 13, no beginning-of-text token), so `EstimateTokenCount` will not change with the engine choice.

### What the packages do per platform

- **Model root (REQ-RAG-053).** Every model (embedding model, reranker, later the local model) is downloaded to `<LocalApplicationData>/TechieRag/models/<model>`: `%LOCALAPPDATA%\TechieRag\models` on Windows, the app container's application-support folder on Mac Catalyst, the app's private `files` folder on Android (`/data/user/0/<package>/files/TechieRag/models`), the app's `Library` area on iOS. Move it with `builder.UseModelRoot(path)` (or `ModelRoot.Set(path)`, or the `TECHIERAG_MODEL_ROOT` environment variable) before the first model loads. A complete copy an older version left next to the assembly is still used, so upgrading does not download again.
...
4. Press **Generate one sentence** (AutomationId `RunGenerateButton`, REQ-FN-059) the same way, accept the model's terms in the dialog, and record the time to first token, tokens per second and peak memory from `GenerationTimingsLabel` as a row of "Local model: runtime comparison" above and in the `TechieRag.Local` cell. Until the owner has chosen the engine it answers with the "no local inference runtime" message.
