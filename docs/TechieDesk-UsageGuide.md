...
5. **Metadata table:** confirm the **Library (N documents)** table columns render — **Name · Type · Size · Chunks · Uploaded · Workspaces · Status** — and that on a 390px-wide viewport the table scrolls inside its container with no page overflow (REQ-UI-021). *(Note: Size may display "—" — see limitations, TR-RAG-004.)*
...
- **Document "Size" may show "—".** The library table has no size field end-to-end yet (library gap TR-RAG-004) — cosmetic, not a failure.
